...
public abstract class SearchLabelProvider extends AppearanceAwareLabelProvider {
	...
	protected final String getLabelWithCounts(Object element, String elementName) {
		...
		// return Messages.format(PatternSearchMessages.SearchLabelProvider_potential_singular, elementName);
		return elementName;
	}
	...
	private ILabelProvider getLabelProvider(Object element) {
		...
		if (!(input instanceof PatternSearchResult))
			return null;

		IMatchPresentation participant= ((PatternSearchResult) input).getSearchParticpant(element);
		if (participant == null)
			return null;
