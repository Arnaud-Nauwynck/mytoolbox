	...
	public static String toString(IWorkingSet[] workingSets) {
		...
		if (workingSets != null && workingSets.length > 0) {
			...
			for (int i= 0; i < workingSets.length; i++) {
				...
				if (firstFound)
					result= Messages.format(PatternSearchMessages.SearchUtil_workingSetConcatenation, new String[] {result, workingSetLabel});
				else {
	...
	public static void warnIfBinaryConstant(IJavaElement element, Shell shell) {
		if (isPrimitiveConstantOrString(element))
			OptionalMessageDialog.open(
				...
				shell,
				PatternSearchMessages.Search_FindReferencesAction_BinPrimConstWarnDialog_title,
				null,
				PatternSearchMessages.Search_FindReferencesAction_BinPrimConstWarnDialog_message,
