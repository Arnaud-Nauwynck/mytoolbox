...
public class PatternSearchResultPage extends AbstractTextSearchViewPage implements IAdaptable {
	...
	private static final String KEY_GROUPING= "fr.an.eclipse.pattern.resultpage.grouping"; //$NON-NLS-1$
	private static final String KEY_SORTING= "fr.an.eclipse.pattern.resultpage.sorting"; //$NON-NLS-1$
	private static final String KEY_LIMIT_ENABLED= "fr.an.eclipse.pattern.resultpage.limit_enabled"; //$NON-NLS-1$
	private static final String KEY_LIMIT= "fr.an.eclipse.pattern.resultpage.limit"; //$NON-NLS-1$

	private static final String GROUP_GROUPING= "fr.an.eclipse.pattern.resultpage.grouping"; //$NON-NLS-1$
	private static final String GROUP_FILTERING = "fr.an.eclipse.pattern.resultpage.filtering"; //$NON-NLS-1$

	private NewSearchViewActionGroup fActionGroup;
	private PatternSearchContentProvider fContentProvider;
	...
	private PatternSearchEditorOpener fEditorOpener= new PatternSearchEditorOpener();

	public PatternSearchResultPage() {
	...
	private void initSortActions() {
		fSortByNameAction= new SortAction(PatternSearchMessages.JavaSearchResultPage_sortByName, this, SortingLabelProvider.SHOW_ELEMENT_CONTAINER);
		fSortByPathAction= new SortAction(PatternSearchMessages.JavaSearchResultPage_sortByPath, this, SortingLabelProvider.SHOW_PATH);
		fSortByParentName= new SortAction(PatternSearchMessages.JavaSearchResultPage_sortByParentName, this, SortingLabelProvider.SHOW_CONTAINER_ELEMENT);
	}

	private void initGroupingActions() {
		fGroupProjectAction= new GroupAction(PatternSearchMessages.JavaSearchResultPage_groupby_project, PatternSearchMessages.JavaSearchResultPage_groupby_project_tooltip, this, PatternSearchResultTreeContentProvider.LEVEL_PROJECT);
		JavaPluginImages.setLocalImageDescriptors(fGroupProjectAction, "prj_mode.gif"); //$NON-NLS-1$
		fGroupPackageAction= new GroupAction(PatternSearchMessages.JavaSearchResultPage_groupby_package, PatternSearchMessages.JavaSearchResultPage_groupby_package_tooltip, this, PatternSearchResultTreeContentProvider.LEVEL_PACKAGE);
		JavaPluginImages.setLocalImageDescriptors(fGroupPackageAction, "package_mode.gif"); //$NON-NLS-1$
		fGroupFileAction= new GroupAction(PatternSearchMessages.JavaSearchResultPage_groupby_file, PatternSearchMessages.JavaSearchResultPage_groupby_file_tooltip, this, PatternSearchResultTreeContentProvider.LEVEL_FILE);
		JavaPluginImages.setLocalImageDescriptors(fGroupFileAction, "file_mode.gif"); //$NON-NLS-1$
		fGroupTypeAction= new GroupAction(PatternSearchMessages.JavaSearchResultPage_groupby_type, PatternSearchMessages.JavaSearchResultPage_groupby_type_tooltip, this, PatternSearchResultTreeContentProvider.LEVEL_TYPE);
	...
	public void showMatch(Match match, int offset, int length, boolean activate) throws PartInitException {
		...
		} else if (getInput() instanceof PatternSearchResult) {
			PatternSearchResult result= (PatternSearchResult) getInput();
	...
	private void showWithMarker(IEditorPart editor, IFile file, int offset, int length) throws PartInitException {
		try {
			...
			throw new PartInitException(PatternSearchMessages.JavaSearchResultPage_error_marker, e);
		}
	}
	...
	private void addSortActions(IMenuManager mgr) {
		if (getLayout() != FLAG_LAYOUT_FLAT)
			return;
		MenuManager sortMenu= new MenuManager(PatternSearchMessages.JavaSearchResultPage_sortBylabel);
	...
	private void updateGroupingActions() {
		fGroupProjectAction.setChecked(fCurrentGrouping == PatternSearchResultTreeContentProvider.LEVEL_PROJECT);
		fGroupPackageAction.setChecked(fCurrentGrouping == PatternSearchResultTreeContentProvider.LEVEL_PACKAGE);
		fGroupFileAction.setChecked(fCurrentGrouping == PatternSearchResultTreeContentProvider.LEVEL_FILE);
		fGroupTypeAction.setChecked(fCurrentGrouping == PatternSearchResultTreeContentProvider.LEVEL_TYPE);
	}
	...
	protected void configureTableViewer(TableViewer viewer) {
		...
		fContentProvider=new PatternSearchResultTableContentProvider(this);
	...
	protected void configureTreeViewer(TreeViewer viewer) {
		...
		fContentProvider= new PatternSearchResultTreeContentProvider(this, fCurrentGrouping);
		viewer.setContentProvider(fContentProvider);
		addDragAdapters(viewer);
	}
	...
	public void init(IPageSite site) {
		...
		menuManager.appendToGroup(IContextMenuConstants.GROUP_PROPERTIES, new Action(PatternSearchMessages.JavaSearchResultPage_preferences_label) {
			@Override
			public void run() {
	 ...
	 *            {@link PatternSearchResultTreeContentProvider}
	 */
	void setGrouping(int grouping) {
		fCurrentGrouping= grouping;
		StructuredViewer viewer= getViewer();
		PatternSearchResultTreeContentProvider cp= (PatternSearchResultTreeContentProvider) viewer.getContentProvider();
	...
	public void restoreState(IMemento memento) {
		...
		int grouping= PatternSearchResultTreeContentProvider.LEVEL_PACKAGE;
	...
	public String getLabel() {
		...
		if (input != null && input.getActiveMatchFilters() != null && input.getActiveMatchFilters().length > 0) {
			if (isQueryRunning()) {
				String message= PatternSearchMessages.JavaSearchResultPage_filtered_message;
				return Messages.format(message, new Object[] { label });

			} else {
				int filteredOut= input.getMatchCount() - getFilteredMatchCount();
				String message= PatternSearchMessages.JavaSearchResultPage_filteredWithCount_message;
		...
		if (firstElement instanceof ICompilationUnit ||
			...
			if (getDisplayedMatchCount(firstElement) == 0) {
				try {
					...
					ExceptionHandler.handle(e, getSite().getShell(), PatternSearchMessages.JavaSearchResultPage_open_editor_error_title, PatternSearchMessages.JavaSearchResultPage_open_editor_error_message);
				}
				return;
			}
		}
